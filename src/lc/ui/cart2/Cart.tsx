...
import {initialState, cartReducer, totalItems, totalValue} from "@/lc/ui/cart2/cartReducer";
                ...
                <ul className="flex flex-col gap-2">
                    {CATALOG.map((p) => (
                        ...
                        >
                            ...
                            <button onClick={()=> dispatch({type:'ADD',payload:{product:p}})} className="text-sm px-3 py-1 rounded bg-gray-800 text-white hover:bg-gray-700">
                                Dodaj
                            </button>
                        </li>
                    ))}
                ...
                <ul className="flex flex-col gap-2">
                    {state.products.map((prod) => <li key={prod.id}
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={()=> dispatch({type:'CHANGE_QTY',payload:{id:prod.id, delta:-1}})} className="w-6 h-6 rounded border border-gray-300 hover:bg-gray-100">
                                −
                            </button>
                            ...
                            <button onClick={()=> dispatch({type:'REMOVE',payload:{id:prod.id}})} className="ml-2 text-xs text-red-500 hover:underline">
                                usuń
                            </button>
                        </div>
                    </li>)}
